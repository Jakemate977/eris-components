...
var MessageComponent = /** @class */ (function () {
	...
	MessageComponent.create = function (data) {
		...
		if (typeof data.type === 'string') {
			...
			//cool1
			var dataType = data.type; // creo que solo se puede usar any ya que unknown no deja
			data.type = Constants_1.MessageComponentTypes[dataType];
		}
		switch (data.type) {
			case Constants_1.MessageComponentTypes.ACTION_ROW: {
				var ActionRow = Promise.resolve().then(function () {
					return __importStar(require('../ActionRow'));
				});
				component = new ActionRow(data);
				break;
			}
			case Constants_1.MessageComponentTypes.BUTTON: {
				var Button = Promise.resolve().then(function () {
					return __importStar(require('../Button'));
				});
